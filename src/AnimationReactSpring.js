...
function AnimationReactSpring() {
  const [toggle, setToggle] = useState(false);
  const icons = [<Icon1 />, <Icon2 />, <Icon3 />];
  const animatedStyles = useSprings(
    3,
    icons.map((_, index) => {
      return {
        to: {
  ...
  const animatedIcons = animatedStyles.map((animatedStyle, index) => {
    return (
      <animated.g
        style={{
...
export default AnimationReactSpring;
